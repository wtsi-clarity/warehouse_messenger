package warehouse_messenger::dao::container_dao;

use Moose;
use Readonly;
use Carp;

use warehouse_messenger::dao::containertypes_dao;
...
has 'type' => (
  ...
  is         => 'ro',
  isa        => 'warehouse_messenger::dao::containertypes_dao',
  lazy_build => 1,
);
...
sub _build_type {
  ...
  return warehouse_messenger::dao::containertypes_dao->new(lims_id => $type_limsid);
}
...
sub _build_well {
  ...
  # study
  my $study = $self->_get_study(lims_id => $sample->project_limsid);
...
sub _get_sample {
  my ($self, $limsid) = @_;
  return warehouse_messenger::dao::sample_dao->new(lims_id => $limsid);
}

# Needs to be renamed to project. There are no studies in Clarity!
sub _get_study {
  my ($self, $limsid) = @_;
  return warehouse_messenger::dao::study_dao->new(lims_id => $limsid);
}
...
warehouse_messenger::dao::container_dao

=head1 SYNOPSIS
  my $container_dao = warehouse_messenger::dao::container_dao->new(lims_id => "1234");
  $container_dao->to_message();
...
=item warehouse_messenger::dao::containertypes_dao;
