...
use_ok('warehouse_messenger::message_handler');

{
  my $mq_handler = warehouse_messenger::message_handler->new();
  isa_ok($mq_handler, 'warehouse_messenger::message_handler');
}

{
  my $mq_handler = warehouse_messenger::message_handler->new();
  my $json_string = '{"__CLASS__":"warehouse_messenger::message::epp-0.33","process_url":"http://clarity.com:1234","step_url":"http://clarity.com:1234/step","timestamp":"2014-11-25 12:06:27","purpose":"sample"}';
  my $message = $mq_handler->_thaw($json_string);

  isa_ok($message, 'warehouse_messenger::message');
...

{
  ...
  my $mocked_mapper = Test::MockObject::Extends->new( warehouse_messenger::mapper->new);
  $mocked_mapper->mock(q{package_name}, sub {
    my ($self, $purpose) = @_;
    return q{warehouse_messenger::} . $message_enhancers->{$purpose}}
  );

  my $mq_handler = warehouse_messenger::message_handler->new(mapper => $mocked_mapper);

  my $enhancer = $mq_handler->_find_enhancer_by_purpose('sample');

  is($enhancer, 'warehouse_messenger::me::sample_enhancer', 'Gets the correct message enhancer');
...
{
  my $mock_mq_handler = Test::MockObject::Extends->new( warehouse_messenger::message_handler->new() );
